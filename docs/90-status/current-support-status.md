...
- **M1 PR4 实验工具（非最终签收）：** [m1-pr4-real-story-ab-playbook.md](./m1-pr4-real-story-ab-playbook.md)；真仓 [m1-pr4-json-java-pair001-runbook.md](./m1-pr4-json-java-pair001-runbook.md)（pair-json-001-r2；attempt2–5 保留）；**pair-json-002 已解除暂停** → [m1-pr4-json-java-pair002-runbook.md](./m1-pr4-json-java-pair002-runbook.md)；记分表含 `run_settled`
- **附录 A（部分隔离）：** Claude Adapter；Rule 触顶；闪断 Resume

## 没有 / 未签（相对手册通路通）

- **M1 真实 Story 签收（PR4 结果）：** 10×A/B 未完成；pair-json-001 有有效故障/闭环证据链；pair-json-002 LAB 已建、Story 待选定后开跑
