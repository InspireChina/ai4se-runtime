...
- **M1 PR4 实验工具（非最终签收）：** [m1-pr4-real-story-ab-playbook.md](./m1-pr4-real-story-ab-playbook.md)；真仓 [m1-pr4-json-java-pair001-runbook.md](./m1-pr4-json-java-pair001-runbook.md)（pair-json-001-r2；attempt2 保留；pair-json-002 暂停）；记分表含 `run_settled`
- **附录 A（部分隔离）：** Claude Adapter；Rule 触顶；闪断 Resume

## 没有 / 未签（相对手册通路通）

- **M1 真实 Story 签收（PR4 结果）：** 10×A/B 未完成；pair-json-001 attempt2 为有效失败证据；r2 待重跑
