...
>
> 本手册定义客户真仓的执行方式：以冻结输入启动一次受控运行，按证据决定是否进入人工接收。
...
除非用户明确要求其他语言，所有面向用户、operator 与审阅者的真实客户需求文档必须以中文为主：包括需求卡、澄清、Discovery、问题清单、影响评估、测试策略、运行摘要、审阅请求与复盘。文件名、代码标识符、路径、命令、错误码和必要的英文术语可以保留英文；不得以英文叙述替代中文主体。该规则应在每个新 LAB 创建时检查，并在冻结输入前复核。
