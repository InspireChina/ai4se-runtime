...
| 3 | **Worker** | 统一执行接口（见 `ai4se-worker-api`） | 实现不在本目录 |
| 4 | **Scheduler** | 队列、依赖、重试（Deferred；见 ADR-0016） | 未做产品 |
...
决策：[ADR-0015](../../adr/0015-five-kernel-objects.md) · 产品定位：[Orchestrator 蓝图](../../../00-product/capability-map.md)
...
3. Worker 是执行面；Capability 是权限与契约面
4. Artifact 是唯一允许跨阶段传递的“产物货币”
5. ExecutionContext 是唯一允许 Worker/Skill 读取的运行时视图（禁止直读全局单例）

## 阅读顺序

1. `00-object-model` → 2. `01` / `02` / `05` → 3. Invariants / Enforcement → 4. 产品蓝图（Orchestrator）
